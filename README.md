...
| `grant_select` | `run-operation` | Grants usage + select on the target schema to a role |
| `clean_stale_models` | `run-operation` | Generates `DROP` statements for objects untouched for N days. Defaults to `dry_run=True` — prints, doesn't drop |
| `load_payment_batch` / `revert_payment_batch` | `run-operation` | Mutates `raw.stripe.payment` to exercise the incremental model, and undoes it |
